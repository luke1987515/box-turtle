...
:: 判斷 diskpart_script.conf 是否存在
IF NOT EXIST diskpart_script.conf (
echo.
echo ### 不存在  diskpart_script.conf        檔案 ###
echo list disk > diskpart_script.conf
echo ###   建立  diskpart_script.conf        檔案 ###
)
...
echo Count number：
echo #####################################################
FOR /F "eol=" %%i in (count.txt) do (echo ###  Count number： %%i%)
echo #####################################################
...
:: 如果 Count number 歸零，清空 result.log 紀錄、刪除 hdd_list_*.txt 
if "%number%"=="0" (
echo %dt% > result.log
if EXIST hdd_list_*.txt ( del /F hdd_list_*.txt )
)
